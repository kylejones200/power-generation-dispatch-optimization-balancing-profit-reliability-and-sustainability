...
def define_generation_sources():
    ...
    sources = {
        "coal": {
            "capacity_mw": 800,
            "min_output_mw": 400,
            "variable_cost_mwh": 35.0,
            "startup_cost": 25000,
            "shutdown_cost": 5000,
            "ramp_rate_mw_hour": 100,
        ...
        "natural_gas_combined_cycle": {
            "capacity_mw": 600,
            "min_output_mw": 200,
            "variable_cost_mwh": 45.0,
            "startup_cost": 8000,
            "shutdown_cost": 1000,
            "ramp_rate_mw_hour": 200,
        ...
        "natural_gas_peaker": {
            "capacity_mw": 250,
            "min_output_mw": 50,
            "variable_cost_mwh": 85.0,
            "startup_cost": 1500,
            "shutdown_cost": 500,
            "ramp_rate_mw_hour": 250,
        ...
        "wind": {
            "capacity_mw": 500,
            "min_output_mw": 0,
            "variable_cost_mwh": 0.0,
            "startup_cost": 0,
            "shutdown_cost": 0,
            "ramp_rate_mw_hour": 500,
...
def optimize_unit_commitment(demand_profile_24h, generation_sources):
    ...
    """
    unit_states = {
        name: {"online": False, "hours_online": 0, "hours_offline": 24}
    ...
    for hour, demand_mw in enumerate(demand_profile_24h):
        ...
        for source_name, specs in generation_sources.items():
            ...
            should_be_online = np.select(
                [
                    source_name == "coal",
                    source_name == "natural_gas_combined_cycle",
                    source_name == "natural_gas_peaker",
                ],
                [demand_mw > 800, demand_mw > 1200, demand_mw > 1600],
                default=True,
            )
...
for hour in range(24):
    base_demand = np.select(
        [6 <= hour <= 9, 17 <= hour <= 21, hour >= 22 or hour <= 5],
        [1850, 2100, 1400],
        default=1650,
    )
...
def dispatch_with_renewable_uncertainty(
    demand_mw, wind_forecast_mw, solar_forecast_mw, generation_sources, hour
):
    """
...
logger.info(
    f"  Renewable Generation: {result['renewable_generation_mw']:.0f} MW ({result['renewable_percentage']:.1f}%)"
)
...
def dispatch_with_emissions_constraint(
    demand_profile_24h, generation_sources, daily_emissions_limit_tons
):
    """
...
emissions_constrained = dispatch_with_emissions_constraint(
    demand_profile, sources, daily_emissions_limit_tons=25000
)
